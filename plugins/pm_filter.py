...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>Title:-{search}</b>/n<b>REQUESTED BY:-{query.from_user.first_name}</b>/n<b>UPLOADED BY:-[movie land](https://t.me/FC_movie_land) ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))

            else:
                await message.reply_text(f"<b>Title:-{search}</b>/n<b>REQUESTED BY:-{query.from_user.first_name}</b>/n<b>UPLOADED BY:-[movie land](https://t.me/FC_movie_land) ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
